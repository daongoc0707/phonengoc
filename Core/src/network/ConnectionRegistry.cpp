...
[[nodiscard]] Status validate_endpoint_override(const Endpoint& endpoint) {
    if (endpoint.host.empty() || endpoint.port == 0U) {
        return fail(ErrorCode::invalid_argument,
                    "network endpoint override requires a host and port");
    }
    for (const char raw_character : endpoint.host) {
        ...
        if (character <= 0x20U || character == 0x7FU ||
            ...
            character == '[' || character == ']') {
            return fail(ErrorCode::invalid_argument,
                        "network endpoint override host is malformed");
        }
    }
    return {};
}
...
Status ConnectionRegistry::set_endpoint_override(
    std::optional<Endpoint> endpoint) {
    if (endpoint.has_value()) {
        auto valid = validate_endpoint_override(*endpoint);
...
Result<OpenedConnection> ConnectionRegistry::open(
    ...
    Entry created;
    {
        ...
        if (endpoint_override_.has_value() &&
            ((created.parsed_url.scheme == Scheme::socket &&
              !created.parsed_url.server_endpoint) ||
             (created.parsed_url.scheme == Scheme::datagram &&
              !created.parsed_url.server_endpoint))) {
            created.parsed_url.host = endpoint_override_->host;
            created.parsed_url.port = endpoint_override_->port;
        }
        created.mode = mode;
        created.timeouts = timeouts;
        const bool long_lived_stream =
            created.parsed_url.scheme == Scheme::socket ||
            created.parsed_url.scheme == Scheme::server_socket;
        created.timeout_ms = timeouts
            ? (long_lived_stream
                   ? kStreamTimeoutEnabledMilliseconds
                   : kTimeoutEnabledMilliseconds)
            : kTimeoutDisabledMilliseconds;
        switch (created.parsed_url.scheme) {
        case Scheme::socket:
            created.kind = created.parsed_url.server_endpoint
...
Status ConnectionRegistry::send_datagram(ConnectionToken token,
    ...
    i32 timeout_ms = 0;
    {
        ...
        timeout_ms = (*found)->timeout_ms;
        if (endpoint_override_.has_value()) {
            packet.peer = *endpoint_override_;
        } else if (packet.peer.host.empty() &&
