...
Status Runtime::configure(std::string runtime_home,
    ...
    app_network_endpoint_configs_.clear();
...
Status Runtime::configure_app_network_endpoint(
    AppId app_id,
    std::string host,
    i32 port) {
    if (!app_id.valid()) {
        return fail(ErrorCode::invalid_argument,
                    "network endpoint requires a valid application ID");
    }

    const bool clearing = host.empty() && port == 0;
    if (!clearing) {
        if (host.empty() || port < 1 || port > 65'535) {
            return fail(ErrorCode::invalid_argument,
                        "network endpoint requires a host and port 1..65535");
        }
        for (const char raw_character : host) {
            ...
            if (character <= 0x20U || character == 0x7FU ||
                ...
                character == '[' || character == ']') {
                return fail(ErrorCode::invalid_argument,
                            "network endpoint host is malformed");
            }
        }
    }
    ...
    std::shared_ptr<ApplicationVM> application_vm;
    {
        std::scoped_lock lock(mutex_);
        if (clearing) {
            app_network_endpoint_configs_.erase(app_id.value);
        } else {
            app_network_endpoint_configs_.insert_or_assign(
                app_id.value,
                AppNetworkEndpointConfig {
                    .host = host,
    ...
    if (application_vm != nullptr) {
        ...
        return application_vm->machine.configure_network_endpoint_override(
            std::move(endpoint));
    }
    return {};
}
...
Status Runtime::start_midlet(SuiteId suite_id,
    ...
    AppFramePacingConfig frame_pacing;
    AppHeapConfig heap_config;
    std::optional<AppNetworkEndpointConfig> network_endpoint_config;
    ...
    u64 lifecycle_token = 0;
    {
        ...
        if (const auto endpoint =
                app_network_endpoint_configs_.find(app_id.value);
            endpoint != app_network_endpoint_configs_.end()) {
            network_endpoint_config = endpoint->second;
        }
    ...
    auto configured = application_vm->machine.configure_network_owner(
        app_id.value);
    if (!configured) return fail_start(configured.error());
    if (network_endpoint_config.has_value()) {
        configured = application_vm->machine
            .configure_network_endpoint_override(network::Endpoint {
                .host = network_endpoint_config->host,
                .port = network_endpoint_config->port,
            });
        if (!configured) return fail_start(configured.error());
    }
