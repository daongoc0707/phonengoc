...
void test_machine_network(const std::string& fixture_jar) {
    ...
    auto override_adapter =
        std::make_shared<phoneme::tests::FakeNetworkAdapter>();
    phoneme::network::ConnectionRegistry override_registry(override_adapter);
    auto second_game_adapter =
        std::make_shared<phoneme::tests::FakeNetworkAdapter>();
    phoneme::network::ConnectionRegistry second_game_registry(
        second_game_adapter);
    require(override_registry.set_endpoint_override(
                phoneme::network::Endpoint {
                    .host = "override.game.test",
                    .port = 19'876,
                }).has_value(),
            "configure per-MIDlet network endpoint override");

    auto overridden_socket = override_registry.open(
        "socket://original.game.test:1234",
        phoneme::network::ConnectionMode::read_write,
        true);
    require(overridden_socket.has_value(),
            "open socket through endpoint override");
    auto overridden_remote = override_registry.remote_endpoint(
        overridden_socket->token);
    require(overridden_remote.has_value() &&
                overridden_remote->host == "override.game.test" &&
                overridden_remote->port == 19'876,
            "outgoing TCP socket uses per-game endpoint");
    auto second_game_socket = second_game_registry.open(
        "socket://second.game.test:7777",
        phoneme::network::ConnectionMode::read_write,
        true);
    require(second_game_socket.has_value(),
            "open socket for second game without an override");
    ...
    require(second_game_remote.has_value() &&
            ...
            "one MIDlet's endpoint does not leak into another MIDlet");
    auto overridden_url = override_registry.url(overridden_socket->token);
    require(overridden_url.has_value() &&
                overridden_url->host == "override.game.test" &&
                overridden_url->effective_port() == 19'876,
            "logical socket URL reflects the actual overridden endpoint");

    auto listener = override_registry.open(
        "serversocket://:2345",
        phoneme::network::ConnectionMode::read_write,
        true);
    require(listener.has_value(),
            "open listener while endpoint override is enabled");
    auto listener_local = override_registry.local_endpoint(listener->token);
    require(listener_local.has_value() && listener_local->port == 2345,
            "endpoint override does not rewrite server socket bind port");

    auto datagram = override_registry.open(
        "datagram://:0",
        phoneme::network::ConnectionMode::read_write,
        true);
    require(datagram.has_value(),
            "open locally bound datagram through endpoint override");
    require(override_registry.send_datagram(
                datagram->token,
                phoneme::network::DatagramPacket {
                    .peer = phoneme::network::Endpoint {
                        .host = "packet.original.test",
                        .port = 3456,
                    },
                    .bytes = {0x01, 0x02, 0x03},
                }).has_value(),
            "send UDP packet through endpoint override");
    auto sent_datagram = override_adapter->last_datagram();
    require(sent_datagram.has_value() &&
                sent_datagram->peer.host == "override.game.test" &&
                sent_datagram->peer.port == 19'876,
            "outgoing UDP packet uses per-game endpoint");

    auto http = override_registry.open(
        "http://web.original.test:8080/path",
        phoneme::network::ConnectionMode::read,
        true);
    require(http.has_value(),
            "open HTTP connection while socket override is enabled");
    auto http_url = override_registry.url(http->token);
    require(http_url.has_value() &&
                http_url->host == "web.original.test" &&
                http_url->effective_port() == 8080,
            "endpoint override leaves HTTP destinations unchanged");
    ...
    require(!override_registry.set_endpoint_override(
            ...
            "reject malformed endpoint override host");
    require(!override_registry.set_endpoint_override(
                 phoneme::network::Endpoint {
                     .host = "valid.test",
                     .port = 0,
                 }).has_value(),
            "reject zero endpoint override port");
    require(override_registry.set_endpoint_override(std::nullopt).has_value(),
            "clear endpoint override");
    auto original_socket = override_registry.open(
        "socket://original.game.test:4321",
        phoneme::network::ConnectionMode::read_write,
        true);
    require(original_socket.has_value(),
            "open socket after clearing endpoint override");
    auto original_remote = override_registry.remote_endpoint(
        original_socket->token);
    require(original_remote.has_value() &&
                original_remote->host == "original.game.test" &&
                original_remote->port == 4321,
            "cleared endpoint override restores game-provided destination");
    override_registry.close_all();
    second_game_registry.close_all();
    require(override_adapter->open_handle_count() == 0U,
            "endpoint override test releases all native handles");
    require(second_game_adapter->open_handle_count() == 0U,
            "second game endpoint test releases all native handles");
