...
/* Redirects future outgoing TCP and UDP connections for one MIDlet. Listener
 * sockets and HTTP(S) remain unchanged. Passing an empty host and port zero
 * clears the override. Existing open connections are not rewritten. */
int32_t phoneme_configure_app_network_endpoint(
    PhoneMERuntimeRef runtime,
    int32_t app_id,
    const char* host,
    int32_t port);
