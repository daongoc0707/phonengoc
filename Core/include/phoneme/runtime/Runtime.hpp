...
struct AppNetworkEndpointConfig final {
    std::string host;
    u16 port {0};
};
...
public:
    ...
    [[nodiscard]] Status configure_app_heap(AppId app_id,
                                            i32 heap_megabytes);
    // Empty host plus port zero clears the per-app TCP/UDP destination.
    [[nodiscard]] Status configure_app_network_endpoint(
        AppId app_id,
        std::string host,
        i32 port);
...
private:
    ...
    std::unordered_map<i32, AppNetworkEndpointConfig>
        app_network_endpoint_configs_;
