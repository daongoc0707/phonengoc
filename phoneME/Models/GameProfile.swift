...
struct GameProfile: Codable, Equatable {
    ...
    struct NetworkEndpoint: Equatable, Sendable {
        let host: String
        let port: Int
    }
    ...
    // Per-game outgoing TCP/UDP endpoint override. Optional values keep
    // profile JSON written by older releases decodable.
    var networkEndpointOverrideEnabled: Bool?
    var networkOverrideHost: String?
    var networkOverridePort: Int?
    ...
    var isNetworkEndpointOverrideEnabled: Bool {
        get { networkEndpointOverrideEnabled ?? false }
        set { networkEndpointOverrideEnabled = newValue ? true : nil }
    }

    var effectiveNetworkOverrideHost: String {
        get {
            Self.normalizedNetworkHost(networkOverrideHost ?? "")
        }
        set {
            let normalized = Self.normalizedNetworkHost(newValue)
            networkOverrideHost = normalized.isEmpty ? nil : normalized
        }
    }

    var effectiveNetworkOverridePort: Int {
        get { networkOverridePort ?? 0 }
        set {
            networkOverridePort = newValue == 0 ? nil : newValue
        }
    }

    var effectiveNetworkEndpoint: NetworkEndpoint? {
        guard isNetworkEndpointOverrideEnabled else { return nil }
        let host = effectiveNetworkOverrideHost
        let port = effectiveNetworkOverridePort
        guard Self.isValidNetworkHost(host), (1...65_535).contains(port) else {
            return nil
        }
        return NetworkEndpoint(host: host, port: port)
    }

    var hasValidNetworkEndpointOverride: Bool {
        !isNetworkEndpointOverrideEnabled || effectiveNetworkEndpoint != nil
    }
    ...
    mutating func normalize() {
        ...
        networkEndpointOverrideEnabled = isNetworkEndpointOverrideEnabled
            ? true
            : nil
        let normalizedNetworkHost = Self.normalizedNetworkHost(
            networkOverrideHost ?? ""
        )
        networkOverrideHost = normalizedNetworkHost.isEmpty
            ? nil
            : normalizedNetworkHost
        if let networkOverridePort {
            self.networkOverridePort = networkOverridePort == 0
                ? nil
                : networkOverridePort
        }
    ...
    private static func normalizedNetworkHost(_ value: String) -> String {
        var result = value.trimmingCharacters(in: .whitespacesAndNewlines)
        for prefix in ["socket://", "datagram://", "tcp://", "udp://"] {
    ...
    private static func isValidNetworkHost(_ value: String) -> Bool {
