...
static atomic_bool gPMBackgroundAudioKeeperActive = ATOMIC_VAR_INIT(false);

void phoneme_ios_set_background_audio_keeper_active(int32_t active) {
    atomic_store_explicit(&gPMBackgroundAudioKeeperActive,
                          active != 0,
                          memory_order_release);
}

static BOOL PMBackgroundAudioKeeperActive(void) {
    return atomic_load_explicit(&gPMBackgroundAudioKeeperActive,
...
#if TARGET_OS_IOS || TARGET_OS_TV
    if (!PMBackgroundAudioKeeperActive()) {
