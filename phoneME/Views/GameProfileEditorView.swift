...
struct GameProfileEditorView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItemGroup(placement: .primaryAction) {
                ...
                .disabled(!profile.hasValidNetworkEndpointOverride)
            }
        }
        .confirmationDialog(
            "Screen size",
    ...
    private var networkSection: some View {
        Section {
            Toggle(
                "Use custom game server",
                isOn: Binding(
                    get: { profile.isNetworkEndpointOverrideEnabled },
                    set: { profile.isNetworkEndpointOverrideEnabled = $0 }
                )
            )

            if profile.isNetworkEndpointOverrideEnabled {
                TextField(
                    "IP address or hostname",
                    text: Binding(
                        get: { profile.networkOverrideHost ?? "" },
                        set: { profile.networkOverrideHost = $0 }
                    )
                )
#if os(iOS)
                ...
                HStack {
                    ...
                    IntegerTextField(
                        value: Binding(
                            get: { profile.networkOverridePort ?? 0 },
                            set: { profile.networkOverridePort = $0 }
                        ),
                        placeholder: "Port",
                        width: 108
                    )
                ...
                if profile.effectiveNetworkEndpoint == nil {
                    Text("Enter a valid IP address or hostname and a port from 1 to 65535.")
                        .font(.footnote)
                        .foregroundStyle(.red)
                }
            }
            ...
            Text(
                "Overrides new outgoing TCP and UDP connections for this game only. Server sockets and HTTP/HTTPS are unchanged. Changing this setting restarts a running game."
            )
        }
    }
