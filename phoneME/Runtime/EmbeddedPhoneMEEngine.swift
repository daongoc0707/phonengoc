...
final class EmbeddedPhoneMEEngine: NSObject {
    ...
    private final class RuntimeContext: @unchecked Sendable {
        ...
        var networkEndpoints: [UUID: GameProfile.NetworkEndpoint] = [:]
        ...
        func setNetworkEndpoint(
            _ endpoint: GameProfile.NetworkEndpoint?,
            for gameID: UUID
        ) {
            if let endpoint {
                networkEndpoints[gameID] = endpoint
            } else {
                networkEndpoints.removeValue(forKey: gameID)
            }
        }
        ...
        func reset() {
            ...
            networkEndpoints.removeAll(keepingCapacity: true)
            nextAppID = 1
        }
    }
    ...
    func launchApplication(
        ...
        framePacingMode: GameProfile.FramePacingMode,
        heapSizeMegabytes: Int,
        networkOverrideHost: String?,
        networkOverridePort: Int?,
        immediateProcessing: Bool,
        parallelScreenRedrawing: Bool,
        autoTranslateToVietnamese: Bool,
        translationProvider: TranslationProvider,
        ...
        let requestedHeapSizeMegabytes =
            GameProfile.resolvedHeapSizeMegabytes(heapSizeMegabytes)
        let requestedNetworkEndpoint: GameProfile.NetworkEndpoint? = {
            guard
                let networkOverrideHost,
                let networkOverridePort,
                (1...65_535).contains(networkOverridePort)
            else {
                return nil
            }
            let host = networkOverrideHost.trimmingCharacters(
                in: .whitespacesAndNewlines
            )
            guard !host.isEmpty else { return nil }
            return GameProfile.NetworkEndpoint(
                host: host,
                port: networkOverridePort
            )
        }()
        framePollingFramesPerSecond = requestedFPS
        fpsMeasurementStartNanoseconds = DispatchTime.now()
        ...
        runtimeQueue.async { [weak self] in
            do {
                ...
                let configureNetworkEndpoint: (Int32) throws -> Void = { appID in
                    let result = loadedAPI.configureApplicationNetworkEndpoint(
                        createdRuntime,
                        appID: appID,
                        host: requestedNetworkEndpoint?.host,
                        port: requestedNetworkEndpoint?.port
                    )
                    guard result == 0 else {
                        throw loadedAPI.failure(
                            status: result,
                ...
                if let existingAppID = context.appIDs[gameID] {
                    ...
                    let networkEndpointChanged =
                        context.networkEndpoints[gameID]
                            != requestedNetworkEndpoint
                    let requiresReplacement =
                        existingState == .destroyed
                            || existingState == .error
                            || networkEndpointChanged

                    if requiresReplacement {
                        if existingState != .destroyed {
                            // Failed VMs and live VMs whose endpoint changed
                            // must release their current sockets before a new
                            // per-game route can be guaranteed.
                        ...
                        try configureNetworkEndpoint(replacementAppID)
                        let result = loadedAPI.startMidlet(
                            createdRuntime,
                            suiteID: resolvedSuiteID,
                            mainClass: mainClass,
                        ...
                        try configureNetworkEndpoint(existingAppID)
                        context.setNetworkEndpoint(
                            requestedNetworkEndpoint,
                            for: gameID
                        )
                    ...
                    try configureNetworkEndpoint(newAppID)
                    let result = loadedAPI.startMidlet(
                        createdRuntime,
                        suiteID: resolvedSuiteID,
                        mainClass: mainClass,
